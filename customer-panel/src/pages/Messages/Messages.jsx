...
// SOCKET.IO APPROACH (Commented out - requires HTTPS backend)
/*
...
function Messages() {
  ...
  /* SOCKET.IO APPROACH (Commented out - uncomment when HTTPS is setup)
  useEffect(() => {
    // Connection event listeners
    socket.on('connect', () => {
      console.log('Customer panel connected to Socket.IO server')
    })

    socket.on('connect_error', (error) => {
      console.error('Customer panel connection error:', error)
    })

    socket.on('disconnect', (reason) => {
      console.log('Customer panel disconnected:', reason)
    })
    ...
    if (message.length == 0) {
      ...
      if (socket.connected) {
        socket.emit('sendMessage', {
          sender: sessionStorage.getItem('name') || 'Customer',
          text: message,
          timestamp: new Date(),
        })
        setMessage('')
